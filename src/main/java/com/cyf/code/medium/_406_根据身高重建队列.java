package com.cyf.code.medium;

import java.util.ArrayList;
import java.util.Arrays;
import java.util.List;

/**
 *假设有打乱顺序的一群人站成一个队列。
 * 每个人由一个整数对(h, k)表示，其中h是这个人的身高，k是排在这个人前面且身高大于或等于h的人数。 编写一个算法来重建这个队列。
 *
 * 注意：
 * 总人数少于1100人。
 *
 * 示例
 *
 * 输入:
 * [[7,0], [4,4], [7,1], [5,0], [6,1], [5,2]]
 *
 * 输出:
 * [[5,0], [7,0], [5,2], [6,1], [4,4], [7,1]]
 *
 * 来源：力扣（LeetCode）
 * 链接：https://leetcode-cn.com/problems/queue-reconstruction-by-height
 * 著作权归领扣网络所有。商业转载请联系官方授权，非商业转载请注明出处。
 */
public class _406_根据身高重建队列 {

    public static int[][] reconstructQueue(int[][] people) {

        /**
         * 思路：1.先对数组进行排序，对身高h进行降序排序，对K进行升序排序，排序的结果 [7,0][7,1][6,1][5,0][5,2][4,4]
         * 2.遍历数组，根据K进行插入,相同的K,后面的会替代前面的位置，前面的位置+1
         */

        Arrays.sort(people, (a, b) -> a[0] == b[0] ? a[1] - b[1] : b[0] - a[0]);
        List<int [] > result = new ArrayList<>();

        for (int [] p : people){
            result.add(p[1], p);
        }

        return result.toArray(new int[result.size()][]);


    }


    public static void main(String[] args) {
        int [] [] people = new int[][]{new int[]{7,0}, new int[]{4,4}, new int[]{7,1}, new int[]{5,0}, new int[]{6,1}, new int[]{5,2}};
        int [] [] result = reconstructQueue(people);

        Arrays.stream(result).forEach(o -> System.out.println(Arrays.toString(o)));
    }
}
